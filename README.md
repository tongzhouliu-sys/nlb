...
*   **👥 双账号并行支持**：支持配置最多两个独立的 NLB 账号，在单次运行中依次独立登录、选座、验证并发送独立的飞书通知。
*   **🎯 智能座位优先级 (Tiers)**：
...
*   **📱 飞书互动卡片推送**：预订结束后自动向飞书群机器人发送 Markdown 格式的通知卡片（绿色代表预约成功，红色代表失败）。
*   **🛡️ 容错性与健壮性**：
...
graph TD
    ...
    L -->|是 / 待核实| M[构建绿色成功卡片 -> 飞书推送]
    L -->|否: 真失败| N[构建红色失败卡片 -> 飞书推送]
    N --> O[抛出异常 & 进程非零状态退出]
```
...
A: 在最后的 Bookings 列表核实步骤中，若因为网络加载缓慢或 NLB 系统的 Upcoming 标签切换超时，导致脚本未能成功进入并确认订单列表，为了防止“明明预订成功但因为页面加载慢而误报失败”，脚本会返回 `None` 判定为“待核实”，此状态不代表预订失败，请用户以 NLB 手机客户端内实际展示为准。
